    ...
    <div class="card col-sm-3" style="min-width: 400px; margin: auto auto; margin-top: 5vh; padding: 15px ; outline: 1px lightgray solid; border-radius: 5px;"">
        <table class="table table-striped">
          <thead style="text-align: center;">
            <th colspan="3">Listagem de Laboratórios</th>
          </thead>
          <tbody>
            <jsp:useBean id="servicos" class="chamaweb.servicos.LaboratorioServices" />
            <c:forEach items="${servicos.todos}" var="laboratorio">
              <tr>
                ...
                <td style="text-align: right; padding-right:20px;"> 
                    <a class="link-light" href="${cp}/processaLaboratorios?acao=prepararAlteracao&id=${laboratorio.id}&idUsuarioAtual=${requestScope.idUsuarioAtual}&tipoUsuarioAtual=${requestScope.tipoUsuarioAtual}" >Editar</a>
                </td>
              </tr>
            </c:forEach>
          </tbody>
        ...
        <form action="${cp}/processaLaboratorios" method="POST">
          ...
          <input type="hidden" name="idUsuarioAtual" value="${requestScope.idUsuarioAtual}" />
          <input type="hidden" name="tipoUsuarioAtual" value="${requestScope.tipoUsuarioAtual}" />
          <button class="btn btn-primary" type="submit">Cadastrar Novo Laboratório</button>
        </form>
    </div>
  </main>
